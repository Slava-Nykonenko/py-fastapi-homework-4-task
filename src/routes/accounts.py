...
async def register_user(
    ...
    Args:
        user_data (UserRegistrationRequestSchema): The registration details
        including email and password.
        background_tasks (BackgroundTasks): FastAPI's background task manager
        used to send emails without blocking the response.
        db (AsyncSession): The asynchronous database session.
        email_sender (EmailSenderInterface): The service used to format and
        send emails.
    ...
    try:
        ...
        activation_link = (
            f"{BASE_URL}/activate/?email={new_user.email}"
...
async def activate_account(
    ...
    Args:
        activation_data (UserActivationRequestSchema): Contains the user's
        email and activation token.
        background_tasks (BackgroundTasks): FastAPI's background task manager
        used to send emails without blocking the response.
        db (AsyncSession): The asynchronous database session.
        email_sender (EmailSenderInterface): The service used to format and
...
async def request_password_reset_token(
    ...
    Args:
        ...
        used to send emails without blocking the response.
        db (AsyncSession): The asynchronous database session.
        email_sender (EmailSenderInterface): The service used to format and send emails.
...
async def reset_password(
    ...
    Args:
        data (PasswordResetCompleteRequestSchema): The request data containing
        the user's email, token, and new password.
        background_tasks (BackgroundTasks): FastAPI's background task manager
        used to send emails without blocking the response.
        db (AsyncSession): The asynchronous database session.
        email_sender (EmailSenderInterface): The service used to format and
